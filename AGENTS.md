...
4. `src/lib/flows.ts` 只负责持久化，**执行引擎在 `entrypoints/canvas/execution.ts`**，因为流程运行需要实时更新 React Flow 节点、变量与日志。条件表达式必须走 `entrypoints/canvas/expression.ts`，不得使用受 MV3 CSP 禁止的 `eval` / `Function`。
...
| `EXTRACT_PAGE_DATA` | 画布 → SW → 目标标签页 | 按 CSS/XPath 选择器提取文本、属性、HTML、列表或表格，结果由流程写入变量 |
...
| AI 工具 | 分组 | 用途 |
|---|---|---|
| `list-procedures` / `get-procedure` | read | 查看现有可编排技能 |
| `list-sites` | read | 查看站点 |
| `create-procedure` / `add-step` / `update-step` / `remove-step` / `set-detect` | write | 创建/编辑可编排技能（含验证技能）；`create-procedure` 必须先提供 `siteId`，可配置返回值契约 |
| `set-output` | write | 配置技能是否返回数据，以及要暴露的提取结果字段 |
| `create-site` / `update-site` | write | 创建/编辑站点 |
| `create-flow` | write | 创建流程 |
| `read-page` / `explore-page` | browser | 读取/探索当前标签页；`explore-page` 生成技能时必须携带目标 `siteId`，成功路径固化为标准动作步骤而非脚本 |
| `ask` | control | 向用户提问澄清 |
...
| 函数 | 文件 | 用途 |
|---|---|---|
| `pageRunOneStep(step)` | `steps.ts` | 执行单步（click/type/goto/wait 等） |
...
| `pageExtractData(options)` | `extract.ts` | 按 CSS/XPath 选择器提取文本、属性、HTML、列表或表格数据；由流程的 `EXTRACT_PAGE_DATA` 消息注入目标标签页 |
...
每次站点执行经 `ExecutionQueue.runOneSite` 调用 `RunContext.run` 并落 `logs`（含 `status` / `message` / `cfWaitedMs` / 触发原因），后台通过 `updateBadgeFromLogs` 与 `notifySummary` 反馈；用户可在设置页"自动化日志"审阅任意历史批次。这是所有其他自学习环节的**共享经验池**。
...
`RunContext.ensureLoggedIn` + `looksLikeLoggedOut` 汇集了四类掉线信号（precheck / step_failed / keyword / url_redirect），任一命中即触发登录技能并**重试一次**。用户在登录技能里补充"未登录关键词"或"登录页 URL 模式"后，全局所有引用该登录技能的站点立即受益。
...
`src/lib/migrate.ts` 用 `schemaVersion` 幂等迁移旧数据（V2 将站点内联步骤转为 Procedure；V3 为站点补齐验证技能引用；V4 为技能补齐 `siteId` 并拆分跨站共享技能；V5 为技能补齐 `output` 返回契约），未来所有结构升级都走同样通道；用户数据在版本更迭中不丢、不需要重录。
...
| 画布运行流程 | React Flow 画布 ▶ | `App.runCurrentFlow` → `executeFlow` → `walk` → `executeNode`（`entrypoints/canvas/execution.ts`） |
| 画布调用技能节点 | 先选网站，再选技能（节点保存 `siteId` + `procedureId`） | `executeNode('procedure')` → `RUN_PROCEDURE` → `runProcedureStandalone`（`run-context.ts`） |
| 技能原子提取与返回 | 技能编辑器添加「提取数据」操作并开启技能返回值 | `extract` 步骤 → `Procedure.output` → `RUN_PROCEDURE.outputs/returnValue` |
| 流程采集与上报 | 画布「提取数据」→「发送请求」节点 | `executeNode('extract'/'http')` → `EXTRACT_PAGE_DATA` / `HTTP_REQUEST` → 流程变量 |
