...
const STEP_UI = {
  waitFor: {
    ...
    extraPh: '可留空',
    timeout: 15000,
    desc: '等到该元素出现再继续。支持 <strong>CSS</strong>（如 <code>.checkin</code>）和 <strong>XPath</strong>（如 <code>//button[contains(.,"签到")]</code>）。',
  },
  click: {
  ...
  waitForText: {
    ...
    extraPh: '例如 签到成功',
    timeout: 10000,
    desc: '反复检查页面文字，直到出现你填的内容。主框一般填 <code>body</code>，附加框填「签到成功」等。',
  },
  type: {
    mainLabel: '输入框选择器',
    extraLabel: '要输入的内容',
...
export function applyStepUI(type, node) {
...
export function renumberSteps(listEl) {
